...
from inferno.layers.losses.EmoNetLoss import (
    create_au_loss, create_emo_loss, EmoNetLoss)
...
def losses_from_cfg(losses_cfg, device):
    ...
    for loss_name, loss_cfg in losses_cfg.items():
        loss_type = loss_name if 'type' not in loss_cfg.keys(
        ) else loss_cfg['type']
...
class FaceReconstructionBase(LightningModule):
    ...
    def __init__(self, cfg,
        ...
        self.render1 = SRenderY(self.cfg.model.image_size, obj_filename=os.path.join(
            get_path_to_assets(), 'head_template.obj'),
            uv_size=256).to(self.device)
        self._setup_losses()
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.learning.optimizer == 'Adam':
            ...
            raise ValueError(
                f"Unsupported optimizer: '{self.cfg.learning.optimizer}'")
        ...
        if 'learning_rate_patience' in self.cfg.learning.keys():
            ...
            scheduler = torch.optim.lr_scheduler.ExponentialLR(
                opt, gamma=self.cfg.learning.learning_rate_decay)
            opt_dict['lr_scheduler'] = scheduler
            schedulers += [scheduler]
        return opt_dict
    ...
    def training_step(self, batch, batch_idx, *args, **kwargs):
        training = True
        # forward pass
        sample = self.forward(batch, train=training,
        ...
        total_loss, losses, metrics = self.compute_loss(
            sample, training=training, validation=False, **kwargs)
        ...
        losses_and_metrics_to_log = {"train/" + k: v.item() if isinstance(v, (torch.Tensor))
                                     else v if isinstance(v, float) else 0. for k, v in losses_and_metrics_to_log.items()}

        if self.logger is not None:
            # self.log_dict(losses_and_metrics_to_log, on_step=False, on_epoch=True, sync_dist=True) # log per epoch, # recommended
            self.log_dict(losses_and_metrics_to_log, on_step=True,
                          on_epoch=True, sync_dist=True)  # log per epoch, # recommended
    ...
    def validation_step(self, batch, batch_idx, *args, **kwargs):
        ...
        # forward pass
        sample = self.forward(batch, train=training,
                              validation=True, teacher_forcing=True, **kwargs)
        # loss
        total_loss, losses, metrics = self.compute_loss(
            sample, training=training, validation=True, **kwargs)

        losses_and_metrics_to_log = {**losses, **metrics}
        # losses_and_metrics_to_log = {"val_" + k: v.item() for k, v in losses_and_metrics_to_log.items()}
        losses_and_metrics_to_log = {"val/" + k: v.item() if isinstance(v, (torch.Tensor))
                                     else v if isinstance(v, float) else 0. for k, v in losses_and_metrics_to_log.items()}
    ...
    def forward(self, batch, training=True, validation=False, render=True, **kwargs):
        ...
        batch = self.exchange(
            batch, ring_size, training=training, validation=validation, **kwargs)
    ...
    def visualize_batch(self, batch, batch_idx, prefix, in_batch_idx=None):
        ...
        for b in in_batch_idx:
            ...
            if 'image_original' in batch.keys():
                image_original = _torch_image2np(
                    batch['image_original'][b]).clip(0, 1)
                visdict['image_original'] += [(image_original *
                                               255.).astype(np.uint8)]
            else:
            ...
            if 'predicted_image' in batch.keys():
                visdict['predicted_image'] += [(_torch_image2np(
                    batch['predicted_image'][b]).clip(0, 1) * 255.).astype(np.uint8)]

            if 'predicted_mask' in batch.keys():
                visdict['predicted_mask'] += [(_torch_image2np(
                    batch['predicted_mask'][b]).clip(0, 1) * 255.).astype(np.uint8)]
            ...
            if "landmarks" in batch.keys():
                if 'fan3d' in batch['landmarks'].keys():
                    ...
                    visdict['landmarks_gt_fan'] += [
                        (landmark_gt_fan * 255.).astype(np.uint8)]
            ...
            visdict['landmarks_pred_fan'] += [
                (landmarks_pred_fan * 255.).astype(np.uint8)]
            ...
            visdict['shape_image'] += [(_torch_image2np(shape_images[b])
                                        * 255.).astype(np.uint8)]
            albedo = torch.zeros(
                [1, 3, self.cfg.model.renderer.uv_size, self.cfg.model.renderer.uv_size], device='cuda')
            ops = self.render1(verts, trans_verts, albedo,
                               h=h, w=w)
    ...
    def compute_loss(self, sample, training, validation):
        ...
        for loss_name, loss_cfg in self.cfg.learning.losses.items():
            ...
            losses["loss_" + loss_name] = self.compute_loss_term(
                sample, training, validation, loss_name, loss_cfg, self.losses)

        for metric_name, metric_cfg in self.cfg.learning.metrics.items():
            assert metric_name not in metrics.keys()
            with torch.no_grad():
                metrics["metric_" + metric_name] = self.compute_loss_term(
                    sample, training, validation, metric_name, metric_cfg, self.metrics)
        ...
        for loss_name, loss_cfg in self.cfg.learning.losses.items():
            ...
            if term is not None:
                if isinstance(term, torch.Tensor) and term.isnan().any():
                    print(
                        f"[WARNING]: loss '{loss_name}' is NaN. Skipping this term.")
                    continue
    ...
    def compute_loss_term(self, sample, training, validation, loss_name, loss_cfg, loss_functions):
        loss_type = loss_name if 'loss_type' not in loss_cfg.keys(
        ) else loss_cfg['loss_type']
        mask_invalid = loss_cfg.get(
            'mask_invalid', False)  # mask invalid frames
        ...
        if mask_invalid:
            if mask_invalid == "mediapipe_landmarks":
                ...
                mask = sample["landmarks_validity"]["mediapipe"].to(
                    dtype=torch.bool)
                # dict_get(sample, mask_invalid)
            else:
                raise ValueError(
                    f"mask_invalid value of '{mask_invalid}' not supported")
        else:
            mask = None
        ...
        if not target_key:  # UNARY LOSSES
            ...
            if isinstance(loss_func, (PhotometricLoss, LipReadingLoss, VGG19Loss, EmoNetLoss)):
                ...
                if masking == "rendered":
                    ...
                    try:
                        gt_mask = dict_get(
                            sample, loss_cfg.get('mask_key', None))
                    except Exception:
                        try:
                            gt_mask = sample["segmentation"][loss_cfg.get(
                                'mask_key', None)]
                        except Exception:
                    ...
                    if masking == "gt":
                        ...
                        image_mask = torch.logical_or(
                            gt_mask, sample["predicted_mask"])
                    elif masking == "gt_rendered_intersection":
                        image_mask = torch.logical_and(
                            gt_mask, sample["predicted_mask"])
                    else:
                        raise ValueError(
                            f"Unsupported masking type: '{masking}'")
            ...
            if isinstance(loss_func, LipReadingLoss):
                ...
                if loss_cfg.get('per_frame', True):
                    ...
                    predicted_mouth = predicted_mouth.view(
                        -1, *predicted_mouth.shape[2:])
                    target_mouth = target_mouth.view(-1,
                                                     *target_mouth.shape[2:])
                    mask = mask.view(-1, *mask.shape[2:])

                loss_value = loss_func(
                    target_mouth, predicted_mouth, mask=mask)
            else:
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=None, **kwargs):
        ...
        losses_and_metrics_to_log = {**losses, **metrics}
        # losses_and_metrics_to_log = {"train_" + k: v.item() for k, v in losses_and_metrics_to_log.items()}
        losses_and_metrics_to_log = {"test/" + k: v.item() if isinstance(v, (torch.Tensor))
                                     else v if isinstance(v, float) else 0. for k, v in losses_and_metrics_to_log.items()}
