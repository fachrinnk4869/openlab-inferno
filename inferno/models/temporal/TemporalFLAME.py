...
def check_for_relative_paths(cfg):
    if not Path(cfg.flame_lmk_embedding_path).is_absolute():
        cfg.flame_lmk_embedding_path = str(
            get_path_to_assets() / cfg.flame_lmk_embedding_path)
    ...
    if 'flame_mediapipe_lmk_embedding_path' in cfg.keys():
        if not Path(cfg.flame_mediapipe_lmk_embedding_path).is_absolute():
            cfg.flame_mediapipe_lmk_embedding_path = str(
                get_path_to_assets() / cfg.flame_mediapipe_lmk_embedding_path)
...
class FlameShapeModel(ShapeModel):
    ...
    def __init__(self, cfg):
        ...
        print("Key cfg:", cfg.keys())
        if 'tex_type' in cfg.keys():
            self.flametex = FLAMETex(cfg)
            print("Using FLAMETex with type:", cfg.tex_type)
        else:
            self.flametex = None
    ...
    def forward(self, sample):
        ...
        if shapecode.shape[-1] < self.flame.cfg.n_shape:
            ...
            shapecode = torch.cat([shapecode, torch.zeros(
                [*shapecode.shape[:-1], missing], device=shapecode.device, dtype=shapecode.dtype)], dim=-1)
        if expcode.shape[-1] < self.flame.cfg.n_exp:
            # pad with zeros
            missing = self.flame.cfg.n_exp - expcode.shape[-1]
            expcode = torch.cat([expcode, torch.zeros(
                [*expcode.shape[:-1], missing], device=expcode.device, dtype=expcode.dtype)], dim=-1)
        ...
        if self.uses_texture() and texcode is not None:
            ...
            albedo = torch.ones(
                [effective_batch_size, 3, 256, 256], device=shapecode.device) * 0.5
        ...
        if T is not None:
            ...
            if landmarks2d_mediapipe is not None:
                landmarks2d_mediapipe = landmarks2d_mediapipe.view(
                    B, T, *landmarks2d_mediapipe.shape[1:])
