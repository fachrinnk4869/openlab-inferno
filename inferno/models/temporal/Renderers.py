...
class FlameLandmarkProjector(Renderer):
    ...
    def forward(self, sample):
        ...
        if T is not None:
            ...
            if self.project_landmarks:
                ...
                if landmarks2d_mediapipe is not None:
                    landmarks2d_mediapipe = landmarks2d_mediapipe.view(
                        B*T, *landmarks2d_mediapipe.shape[2:])
        ...
        if self.project_landmarks:
            if landmarks2d is not None:
                predicted_landmarks = util.batch_orth_proj(landmarks2d, cam)[
                    :, :, :2]
            if landmarks3d is not None:
                predicted_landmarks_3d = util.batch_orth_proj(landmarks3d, cam)[
                    :, :, :2]
            if landmarks2d_mediapipe is not None:
                predicted_landmarks_mediapipe = util.batch_orth_proj(
                    landmarks2d_mediapipe, cam)[:, :, :2]
        ...
        if self.project_landmarks:
            ...
            if landmarks3d is not None:
                predicted_landmarks_3d[:, :, 1:] = - \
                    predicted_landmarks_3d[:, :, 1:]
            if landmarks2d_mediapipe is not None:
                predicted_landmarks_mediapipe[:, :, 1:] = - \
                    predicted_landmarks_mediapipe[:, :, 1:]

        # batch temporal unsqueeze
        if T is not None:
            # predicted_images = predicted_images.view(B, T, *predicted_images.shape[1:])
            predicted_mask = predicted_mask.view(
                B, T, *predicted_mask.shape[1:])
            if self.project_landmarks:
                if landmarks2d is not None:
                    predicted_landmarks = predicted_landmarks.view(
                        B, T, *predicted_landmarks.shape[1:])
                if landmarks3d is not None:
                    predicted_landmarks_3d = predicted_landmarks_3d.view(
                        B, T, *predicted_landmarks_3d.shape[1:])
                if landmarks2d_mediapipe is not None:
                    predicted_landmarks_mediapipe = predicted_landmarks_mediapipe.view(
                        B, T, *predicted_landmarks_mediapipe.shape[1:])
...
class FlameRenderer(Renderer):
    ...
    def forward(self, sample):
        ...
        if "albedo" in sample:
            ...
            albedo = torch.zeros(B, 3, self.render.uv_size,
                                 self.render.uv_size).to(verts.device)
            if T is not None:
        ...
        # batch temporal unsqueeze
        if T is not None:
            predicted_images = predicted_images.view(
                B, T, *predicted_images.shape[1:])
            predicted_mask = predicted_mask.view(
                B, T, *predicted_mask.shape[1:])
            if self.project_landmarks:
                if landmarks2d is not None:
    ...
    def render_coarse_shape(self, sample, indices=None, **kwargs):
        ...
        coarse_shape_rendering = self.render.render_shape(
            verts, trans_verts, **kwargs)
...
class FixedViewFlameRenderer(FlameRenderer):
    ...
    def __init__(self, cfg):
        ...
        if self.cut_out_mouth:
            self.mouth_crop_width = cfg.get(
                "mouth_crop_width", 96)  # the default of SPECTRE
    ...
    def forward(self, sample, train=False, input_key_prefix="gt_", output_prefix="predicted_", vertex_keyword="vertices", **kwargs):
        ...
        cams = self.fixed_cams.unsqueeze(0).unsqueeze(
            0).expand(B, T, C, self.fixed_cams.shape[-1])

        # light
        # shape [B, T * #num cams, 9, 3]
        # lightcode = self.fixed_lightcode.unsqueeze(0).unsqueeze(0).repeat(B, T, 1, 1, 1)
        lightcode = self.fixed_lightcode.unsqueeze(0).unsqueeze(
            0).expand(B, T, C, *self.fixed_lightcode.shape[-2:])
        ...
        if self.project_landmarks:
            ...
            landmarks_2d_posed = self.shape_model._vertices2landmarks2d(verts_posed.view(
                B * T * C, *verts_posed.shape[3:]), full_pose.view(B * T * C, *full_pose.shape[3:]))
            landmarks_2d_posed = landmarks_2d_posed.view(
                B, T * C, *landmarks_2d_posed.shape[1:])
        ...
        for key in rendering_sample:
            ...
            rendering_sample[key] = rendering_sample[key].reshape(
                B, T * C, *dims_after_cam)
        ...
        for ci, cam_name in enumerate(self.cam_names):
            ...
            predicted_vid = rendering_sample["predicted_" +
                                             self.output_image_keyword][:, ci::C, ...]
            if self.apply_mask:
                predicted_vid = predicted_vid * \
                    rendering_sample["predicted_mask"][:, ci::C, ...]
    ...
    def cut_mouth(self, images, landmarks, convert_grayscale=True):
        ...
        for frame_idx, frame in enumerate(images):
            ...
            # but I don't like it
            window_margin = min(self.mouth_window_margin //
                                2, frame_idx, len(landmarks) - 1 - frame_idx)
            ...
            smoothed_landmarks = landmarks[frame_idx -
                                           window_margin:frame_idx + window_margin + 1].mean(dim=0)
            ...
            smoothed_landmarks += landmarks[frame_idx].mean(
                dim=0) - smoothed_landmarks.mean(dim=0)

            # extract the mouth landmarks and get their mean
            center_x, center_y = torch.mean(
                smoothed_landmarks[self.mouth_landmark_start_idx:self.mouth_landmark_stop_idx], dim=0)
...
def cut_mouth_vectorized(images,
    ...
    with torch.no_grad():
        ...
        # landmarks_t = landmarks_t.unsqueeze(0)
        # smooth with temporal convolution
        temporal_filter = torch.ones(
            mouth_window_margin, device=images.device) / mouth_window_margin
        # pad the the landmarks
        landmarks_t_padded = F.pad(
            landmarks_t, (mouth_window_margin // 2, mouth_window_margin // 2), mode='replicate')
        # convolve each channel separately with the temporal filter
        num_channels = landmarks_t.shape[1]
        smooth_landmarks_t = F.conv1d(landmarks_t_padded,
                                      temporal_filter.unsqueeze(0).unsqueeze(0).expand(
                                          num_channels, 1, temporal_filter.numel()),
                                      groups=num_channels, padding='valid'
                                      )
        smooth_landmarks_t = smooth_landmarks_t[..., 0:landmarks_t.shape[-1]]

        # reshape back to the original shape
        smooth_landmarks_t = smooth_landmarks_t.permute(
            0, 2, 1).view(landmarks.shape)
        smooth_landmarks_t = smooth_landmarks_t + \
            landmarks.mean(dim=2, keepdims=True) - \
            smooth_landmarks_t.mean(dim=2, keepdims=True)

        # #2) get the mouth landmarks
        mouth_landmarks_t = smooth_landmarks_t[...,
                                               mouth_landmark_start_idx:mouth_landmark_stop_idx, :]
        ...
        grid = torch.stack(torch.meshgrid(torch.linspace(-height, height, mouth_crop_height).to(images.device) / (images.shape[-2] / 2),
                           ...
                           dim=-1)
        grid = grid[..., [1, 0]]
        grid = grid.unsqueeze(0).unsqueeze(
            0).repeat(*images.shape[:2], 1, 1, 1)
        ...
        center_xy = torch.cat([center_x_t, center_y_t],
                              dim=-1).unsqueeze(-2).unsqueeze(-2)
